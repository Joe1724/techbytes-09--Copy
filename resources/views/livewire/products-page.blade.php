<div class="w-full mx-auto bg-purple-300">
<div class="w-full bg-purple-300 max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
    <section class="py-10 rounded-lg bg-gray-50 font-poppins dark:bg-gray-800">
      <div class="px-4 py-4 mx-auto max-w-7xl lg:py-6 md:px-6">
        <div class="flex flex-wrap mb-24 -mx-3">
          <div class="w-full pr-2 lg:w-1/4 lg:block">
            <div class="p-4 mb-5 bg-white border border-gray-200 dark:border-gray-900 dark:bg-gray-900">
              <h2 class="text-2xl font-bold dark:text-black"> Categories</h2>
              {{-- {{ json_encode($selected_category) }} --}}
              <div class="w-16 pb-2 mb-6 border-b border-rose-600 dark:border-gray-400"></div>
              <ul>

                @foreach ($category as $categories)
                <li class="mb-4" wire:key="{{ $categories->id }}">
                    <label for="{{ $categories->slug }}" class="flex items-center dark:text-gray-400 ">
                      <input type="checkbox" wire:model.live="selected_category" id="{{ $categories->slug }}" value="{{ $categories->id }}" class="w-4 h-4 mr-2">
                      <span class="text-lg">{{ $categories->name }}</span>
                    </label>
                  </li>
                @endforeach
              </ul>
            ...
            <div class="p-4 mb-5 bg-white border border-gray-200 dark:bg-gray-900 dark:border-gray-900">
              ...
              <div>
                ...
                <input type="range" wire:model.live='price_range' class="w-full h-1 mb-4 bg-blue-100 rounded appearance-none cursor-pointer" max="500000" value="500000" step="1000">
                <div class="flex justify-between ">
                  <span class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency(500, 'PHP') }}</span>
                  <span class="inline-block text-lg font-bold text-blue-400 ">{{ Number::currency(500000, 'PHP') }}</span>
                </div>
              </div>
            </div>
          </div>
                    ...
                    <div class="w-full px-3 mb-6 sm:w-1/2 md:w-1/3" wire:key="{{ $product->id }}">
                        ...
                        <div class="relative bg-gray-200">
                            <a href="/products/{{ $product->slug }}" class="">
                                ...
                                @else
                                    <img src="{{ url('storage/default-image.jpg') }}" alt="Default Image" class="object-cover w-full h-56 mx-auto">
                                @endif
                            </a>
                        </div>
                        <div class="p-3 ">
                            ...
                            <p class="text-lg ">
                            <span class="text-green-600 dark:text-green-600">{{ Number::currency($product->price, 'PHP') }}</span>
                            </p>
                        </div>
                        <div class="flex justify-center p-4 border-t border-gray-300 dark:border-gray-700">

                            <a wire:click.prevent='addToCart({{ $product->id }})' href="#" class="flex items-center space-x-2 text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-300">
